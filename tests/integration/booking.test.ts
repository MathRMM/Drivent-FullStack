...
describe("GET /booking/:roomId", () => {
  ...
  describe("when token is valid", () => {
    it("should respond with status 400 when given room id is not valid ", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
      ...
      const response = await server.get(`/booking/${roomId}`).set("Authorization", `Bearer ${token}`);

      expect(response.status).toEqual(httpStatus.BAD_REQUEST);
    });

    it("should respond with status 404 when there isnt a room for given room id", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
    ...
    it("should respond with status 200 when there is a room for given room id", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
      ...
      expect(response.status).toEqual(httpStatus.OK);
      expect(response.body).toEqual({ occupancy: 1 });
    });
  });
});
